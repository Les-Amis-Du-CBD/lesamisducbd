...
export const prestaCheckoutService = {
    ...
    async createCustomer(userData, isGuest = 0) {
        ...
        if (existingData) {
            // SYNC GROUP: If user is "buraliste" in Next.js but "client" in PrestaShop, we must upgrade them
            if (parseInt(existingData.id_default_group) !== defaultGroup) {
                console.log(`[PrestaCheckoutService] Group mismatch for ${userData.email}: PS=${existingData.id_default_group} vs Next=${defaultGroup}. Updating...`);

                // For a PUT to be safe, we should include the mandatory fields we just fetched
                const xmlUpdate = `<?xml version="1.0" encoding="UTF-8"?>
<prestashop xmlns:xlink="http://www.w3.org/1999/xlink">
    <customer>
        <id>${existingData.id}</id>
        <email><![CDATA[${existingData.email}]]></email>
        <firstname><![CDATA[${existingData.firstname}]]></firstname>
        <lastname><![CDATA[${existingData.lastname}]]></lastname>
        <id_default_group>${defaultGroup}</id_default_group>
    </customer>
</prestashop>`;
                try {
                    await fetchPrestaXml(`/customers/${existingData.id}`, xmlUpdate, 'PUT');
                    console.log(`[PrestaCheckoutService] Customer ${existingData.id} successfully updated to group ${defaultGroup}`);
                    return { ...existingData, id_default_group: defaultGroup };
                } catch (err) {
                    console.error("[PrestaCheckoutService] Failed to update customer group, continuing with old group", err);
                }
            }
            return existingData; // returns {id, id_default_group, ...}
        }
        ...
        const genderNode = userData.id_gender ? `<id_gender><![CDATA[${escapeXml(userData.id_gender)}]]></id_gender>` : '';
